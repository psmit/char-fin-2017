...
fn=$(basename $f .train)
for d in 0.005 0.008 0.0001 0.0002; do
#for d in 0.001; do
...
ret=$(sbatch -p $partitions -t 5-00:00:00 --job-name=varikn -e $dir/log-%j.out -o $dir/log-%j.out --mem-per-cpu $mem $depend -- common/train_varikn_background.sh $dir $fn $d)
